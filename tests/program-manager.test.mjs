...
const source = app.slice(app.indexOf('const programManagerView ='), app.indexOf('/* ---------------- 프로그램 편집'));
...
function ui({ role = 'admin' } = {}) {
  ...
  let html = '', data = fixture(), fail = false, confirm = true, disposeCount = 0;
  const node = id => {
    if (!nodes.has(id)) nodes.set(id, { value: '', innerHTML: '', textContent: '', disabled: false, isConnected: true, dataset: {}, setAttribute(k, v) { this[k] = v; }, focus() { this.focused = true; } });
    return nodes.get(id);
  };
  ...
  const context = vm.createContext({
    ...
    api: async (method, url, body) => { calls.push({ method, url, body }); if (method === 'GET') return { programs: structuredClone(data) }; if (fail) throw new Error('저장 실패'); return {}; },
  });
