...
test('student screen renders lesson navigation and the three destinations without fake example submissions',async()=>{const u=ui();await u.open();for(const text of ['오늘 수업','내 제출물','내 활동 기록','활동하기','자료·활동지','제출 게시판','관찰 웹앱'])assert.ok(u.html.includes(text));assert.ok(!u.html.includes('학생 A'));assert.ok(u.html.includes('자동 저장되지 않습니다'));});
test('submission board includes camera capture, file preview controls, teacher visibility and honest record state',async()=>{const u=ui();await u.board();for(const text of ['capture="environment"','내 결과물 올리기','sc-file-input','선생님에게 제출','기록 미연결'])assert.ok(u.html.includes(text));});
