...
test('actual file-confirm endpoint serializes duplicate confirmations and rejects changed metadata',async()=>{
 ...
 const deps={'node:crypto':require('node:crypto'),'./course-plan':require('../lib/course-plan'),'./password':{},'./cookies':{},'./worksheet-headers':require('../lib/worksheet-headers'),'./storage':{storageEnabled:true},'./db':db,'./auth':{getSessionUser:async()=>({user:{id:1,role:'admin'}}),roleLevel:()=>2}};
