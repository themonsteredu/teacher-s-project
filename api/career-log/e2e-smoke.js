...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const response = await fetch(`https://${host}/api`, {
      ...
      body: JSON.stringify({ career_log_route: 'ingest', board_code: boardCode, student_id: studentId, process: '삼국·가야 문화유산을 관찰하고 모둠에서 조사할 데이터 질문을 정함', artifact: '모둠 데이터 질문 · 첨성대: 문화유산을 서로 비교하려면 어떤 정보를 모아야 할까?', reflection: '사진을 관찰한 뒤 자료로 확인할 질문을 정했다.', source_event_id: sourceEventId, raw_data: { lesson: 1, era: 'three-kingdoms', selected_heritage: '첨성대', data_question: '문화유산을 서로 비교하려면 어떤 정보를 모아야 할까?', activity: 'heritage-question-card', e2e: true } }),
    });
