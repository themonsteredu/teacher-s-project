...
async function renderStudentClass(code, join, careerStudentId) {
  ...
  function render() {
    ...
    <aside class="sc-career-banner">${icon('file')}<div><strong>활동 기록과 연결</strong><p>${s.record?.enabled?'이 차시의 결과물을 제출하면 진로기록에도 저장합니다. 실제 저장 결과는 내 진로기록에서 확인하세요.':'이 차시는 게시판 제출만 받습니다. 진로기록 저장은 선생님의 차시 설정을 따릅니다.'}</p></div><span>${s.record?.enabled?'진로기록 사용':'게시판 제출'}</span></aside></div></main></div>`;
    bind();
  }
  async function feed(append=false) {
    ...
    try { if(page==='records'){const next=await sbRequest('GET',`${base}/career-records?${p}`);if(page===selectedPage)data={...data,careerRecords:append?[...(data.careerRecords||[]),...next.records]:next.records,next:next.next};return;} const next=await sbRequest('GET',`${base}/submissions?${p}`);if(page!==selectedPage||scope!==selectedScope||current!==selectedLesson)return;data={...next,posts:append?[...data.posts,...next.posts]:next.posts}; }
  ...
  function bind() {
    ...
    root.querySelectorAll('[data-career-retry]').forEach(b=>b.onclick=async()=>{if(busy)return;busy=true;b.disabled=true;try{const r=await sbRequest('POST',`${base}/career-records/${b.dataset.careerRetry}/retry`,{});toast(r.state==='saved'?'진로기록 저장을 확인했습니다.':'저장 연결을 확인하지 못했습니다. 제출 원본은 보관되어 있습니다.',r.state!=='saved');}catch(e){toast(e.message,true);}finally{busy=false;await feed();}});
    root.querySelectorAll('[data-career-file]').forEach(b=>b.onclick=async()=>{b.disabled=true;try{const r=await sbRequest('GET',`${base}/career-records/${b.dataset.careerFile}/file`);const a=document.createElement('a');a.href=r.url;a.target='_blank';a.rel='noopener noreferrer';a.click();}catch(e){toast(e.message,true);}finally{b.disabled=false;}});
  ...
  async function send() {
    ...
    try {
      ...
      const submitted = await sbRequest('POST',`${base}/posts`,pending.snapshot);
