...
window.AccountManager = {
  init({request,message,clearStudent,refreshStudent}) {
    ...
    function renderAccess(){const info=schoolsData.find(s=>s.id===schoolId);$('access-form').hidden=!schoolId||$('manager-pane').dataset.admin!=='true';if(!info)return;$('open-lab').checked=(info.openedTo||[]).includes('moakit-lab');$('access-status').textContent=info.via==='open'?'모아랩에서 이 학교를 모아허브에 열어 주어 관리자 권한으로 관리합니다.':info.via==='admin'?'담당자로 지정돼 있지 않지만 관리자 권한으로 관리합니다.':'';}
