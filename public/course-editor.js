...
const blankCourseSession = () => ({title:'',minutes:40,sourceLessons:[],bridge:'',assets:Object.fromEntries(Object.keys(COURSE_SLOTS).map(k=>[k,{url:'',fileId:''}])),record:{mode:'none',completion:'',original:'',process:''}});
...
function courseProblems(plan) {
  ...
  plan.variants.forEach(v=>v.sessions.forEach((s,i)=>{
    ...
    Object.entries(COURSE_SLOTS).forEach(([k,name])=>{if(!s.assets[k].url&&!s.assets[k].fileId) out.push(`${label}: ${name}`);});
...
function coursePreviewHtml(plan, files, index=0) {
  ...
  }).join('')}</div><p class="small muted">${s.record.mode==='submission'?`제출 시 기록 설계: ${esc(s.record.process||'설명 미입력')} · 실제 저장 연동은 별도 확인`:'Career Log 기록 미설정'}</p></section>`).join('')}</div>`;
}
route(/^#\/manage\/(\d+)$/, async id => {
  ...
  function draw(){
    ...
    shell('수업 등록',`<div class="course-editor"><aside class="ce-sidebar"><a class="ce-brand" href="#/">MOAKIT <span>수업허브</span></a><a href="#/manage">수업 목록</a><a class="active" href="#/manage/${id}">＋ 수업 등록</a><a href="#/myclass">내 수업</a><a href="#/resources/${id}">공통 자료 관리</a><p>좋은 수업이<br>더 많은 아이들에게</p></aside><div class="ce-main">
    <header class="ce-head"><div><h1>수업 등록</h1><p>웹앱 하나에 수업 구성과 자료를 연결하세요.</p></div><span class="ce-status" id="ce-status" role="status">${draft.dirty?'저장하지 않은 변경사항':'구성 편집'}</span></header>
    ...
    <section class="ce-section" id="ce-step-2"><h2>${esc(v.name)} 자료 연결</h2><p class="muted">공통 파일은 여러 구성에서 재사용합니다. 달라지는 자료만 새로 연결하세요.</p><div class="ce-table-scroll"><table class="ce-table ce-assets"><thead><tr><th>차시</th>${Object.values(COURSE_SLOTS).map(n=>`<th>${n}</th>`).join('')}</tr></thead><tbody>${v.sessions.map((s,i)=>`<tr><th>${i+1}차시</th>${Object.entries(COURSE_SLOTS).map(([k,name])=>{const a=s.assets[k];return `<td><button type="button" class="ce-asset-button" data-asset="${i}:${k}">${a.fileId?'파일 연결됨':a.url?'주소 연결됨':'＋ 파일·링크'}</button><small>${esc(a.fileId?data.files.find(f=>String(f.id)===a.fileId)?.name||'파일 확인 필요':a.url?'연결 주소 수정':'아직 미등록')}</small></td>`;}).join('')}</tr>`).join('')}</tbody></table></div><a class="ce-library-link" href="#/resources/${id}">공통 자료·원본 차시 관리 열기 →</a></section>
    <section class="ce-section" id="ce-step-3"><h2>활동 기록 설계</h2><p class="muted">정답과 분리해 실제 제출한 학생 원본을 남깁니다. 이 설정만으로 웹앱의 저장 기능이 켜지지는 않습니다.</p>${v.sessions.map((s,i)=>`<details class="ce-record"><summary>${i+1}차시 · ${esc(s.title||'활동 제목 미입력')} <span>${s.record.mode==='submission'?'제출 기록 설계':'기록 미설정'}</span></summary><label>기록 방식<select data-record="${i}" data-field="mode"><option value="none" ${s.record.mode==='none'?'selected':''}>기록하지 않음</option><option value="submission" ${s.record.mode==='submission'?'selected':''}>학생 제출 시 기록</option></select></label><div class="ce-record-fields">${field('제출 완료 조건',s.record.completion,`data-record="${i}" data-field="completion" placeholder="예: 관찰 내용과 결과물 제출" maxlength="500"`)}${field('저장할 학생 원본',s.record.original,`data-record="${i}" data-field="original" placeholder="예: 식물 이름, 관찰 특징, 학생 결과물" maxlength="1000"`)}${field('한 줄 활동 설명',s.record.process,`data-record="${i}" data-field="process" placeholder="예: 식물을 관찰하고 특징을 기록함" maxlength="300"`)}</div><p class="ce-note">실제 저장 연결: 별도 연동 검증 필요 · 점수·정답 여부는 완료 조건으로 사용하지 않습니다.</p></details>`).join('')}</section>
  ...
  function bind(){
    ...
    root.querySelectorAll('[data-step]').forEach(b=>b.onclick=()=>document.getElementById(`ce-step-${b.dataset.step}`).scrollIntoView({behavior:'smooth',block:'start'}));
    ...
    root.querySelector('#ce-add-variant').onclick=()=>{
      ...
      const modal=openModal('<h3>운영 구성 추가</h3><label>구성 이름<input id="cv-name" placeholder="예: 집중 탐구 과정" maxlength="80"></label><label>차시 수<input id="cv-count" type="number" min="1" max="50" value="1"></label><p class="small muted">차시 수만 줄여도 자료가 자동 압축되지는 않습니다. 각 차시에 맞춰 자료를 연결하세요.</p><div class="m-actions"><button class="btn btn-ghost" id="cv-cancel">취소</button><button class="btn btn-primary" id="cv-add">추가</button></div><p id="cv-error" role="status"></p>');
      modal.querySelector('#cv-cancel').onclick=()=>modal.remove();
      modal.querySelector('#cv-add').onclick=()=>{const name=modal.querySelector('#cv-name').value.trim(),n=Number(modal.querySelector('#cv-count').value);if(!name||!Number.isInteger(n)||n<1||n>50){modal.querySelector('#cv-error').textContent='구성 이름과 1~50 사이 차시 수를 입력하세요.';return;}draft.plan.variants.push({id:crypto.randomUUID(),name,sessions:Array.from({length:n},blankCourseSession)});draft.selected=draft.plan.variants.length-1;changed();modal.remove();draw();};
    };
