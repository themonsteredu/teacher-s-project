...
route(/^#\/manage\/(\d+)$/, async id => {
  ...
  function lessonEditor(v) {
    ...
    <section class="ce-detail-section"><h3>진로기록 설정</h3><label>기록 방법<select data-record="${i}" data-field="mode" ${s.submissions.enabled?'':'disabled'}><option value="none" ${s.record.mode==='none'?'selected':''}>게시판 제출만 · 진로기록 설정 없음</option><option value="submission" ${s.record.mode==='submission'?'selected':''}>Hub 학생 제출의 진로기록 설정</option></select></label><p id="ce-record-hint" class="muted">${s.submissions.enabled?'학교와 학생 계정을 연결한 수업에서, 이 차시의 실제 제출물을 기록합니다.':'학생 제출을 켜면 진로기록을 설계할 수 있습니다.'}</p><div class="ce-record-fields" id="ce-record-fields" ${s.record.mode==='submission'?'':'hidden'}>${field('학생 제출 안내',s.record.completion,`data-record="${i}" data-field="completion" placeholder="예: 식물 관찰 내용과 활동지를 제출하세요." maxlength="500"`)}${field('받을 학생 결과물',s.record.original,`data-record="${i}" data-field="original" placeholder="예: 식물 이름, 관찰 특징, 활동지 사진" maxlength="1000"`)}${field('기록에 남길 한 줄 활동 설명',s.record.process,`data-record="${i}" data-field="process" placeholder="예: 식물을 관찰하고 특징을 기록함" maxlength="300"`)}<div class="ce-record-example"><span>활동 설명 미리보기</span><p id="ce-record-example">${esc(s.record.process||'한 줄 활동 설명을 입력하세요.')}</p><small>학생의 글·첨부파일과 함께 남길 활동 설명입니다. 내 수업에서 학교를 지정하고 기록 저장을 켜면 실제 제출 시 저장합니다.</small></div></div><p class="ce-note">Hub 게시판에 제출한 사진·문서·글을 기록합니다. 외부 웹앱의 결과는 학생이 결과물로 제출하며, 앱을 열기만 해서는 기록되지 않습니다.</p></section></div></div>`;
  }
  function draw(){
