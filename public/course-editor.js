...
function coursePreviewHtml(plan, files, index=0) {
  ...
  }).join('')}</div><p class="small muted">${s.submissions?.enabled?`학생 제출: ${s.submissions.types.map(t=>({photo:'사진',document:'PDF·문서',text:'글'}[t])).join(' · ')} / ${s.submissions.sharing==='class'?'교사 확인 후 우리 반 공개':'선생님만 열람'}`:'학생 제출 받지 않음'}</p><p class="small muted">${s.record.mode==='submission'?`진로기록 설계: ${esc(s.record.process||'설명 미입력')} · 실제 저장 연결은 준비 중`:'진로기록 설계 없음'}</p>${s.record.mode==='submission'?`<p class="small">제출 안내: ${esc(s.record.completion||'미입력')}<br>받을 내용: ${esc(s.record.original||'미입력')}</p>`:''}</section>`).join('')}</div>`;
}
route(/^#\/manage\/(\d+)$/, async id => {
  ...
  function lessonEditor(v) {
    ...
    <section class="ce-detail-section"><h3>진로기록 설계</h3><label>기록 계획<select data-record="${i}" data-field="mode" ${s.submissions.enabled?'':'disabled'}><option value="none" ${s.record.mode==='none'?'selected':''}>게시판 제출만 · 진로기록 설계 없음</option><option value="submission" ${s.record.mode==='submission'?'selected':''}>Hub 학생 제출의 진로기록 설계</option></select></label><p id="ce-record-hint" class="muted">${s.submissions.enabled?'기록할 내용을 미리 설계합니다. Hub 제출물의 진로기록 저장 연결은 준비 중입니다.':'학생 제출을 켜면 진로기록을 설계할 수 있습니다.'}</p><div class="ce-record-fields" id="ce-record-fields" ${s.record.mode==='submission'?'':'hidden'}>${field('제출 안내 계획',s.record.completion,`data-record="${i}" data-field="completion" placeholder="예: 식물 관찰 내용과 활동지를 제출하세요." maxlength="500"`)}${field('받을 학생 원본 계획',s.record.original,`data-record="${i}" data-field="original" placeholder="예: 식물 이름, 관찰 특징, 활동지 사진" maxlength="1000"`)}${field('기록에 남길 한 줄 활동 설명 계획',s.record.process,`data-record="${i}" data-field="process" placeholder="예: 식물을 관찰하고 특징을 기록함" maxlength="300"`)}<div class="ce-record-example"><span>활동 설명 미리보기</span><p id="ce-record-example">${esc(s.record.process||'한 줄 활동 설명을 입력하세요.')}</p><small>학생의 글·첨부파일과 함께 남길 활동 설명을 설계합니다. 이 설정만으로 진로기록이 저장되지는 않습니다.</small></div></div><p class="ce-note">Hub 게시판의 사진·문서·글 제출은 사용할 수 있습니다. 진로기록 자동 저장과 외부 웹앱 내부 결과 수집은 별도 연결이 필요합니다.</p></section></div></div>`;
  }
  function draw(){
