...
const { seedCurriculum, CURRICULUM } = require('./curriculum');
...
// A single checked-out connection is required for atomic submission + receipt writes.
async function transaction(work) {
  ...
  try {
    ...
    const result = await work({ q: query, one: async (sql, params) => (await query(sql, params))[0] || null });
    await client.query('COMMIT');
    return result;
  } catch (e) {
    await client.query('ROLLBACK').catch(() => {});
...
CREATE TABLE IF NOT EXISTS lessons (
  ...
  topic TEXT NOT NULL DEFAULT ''             -- 차시 주제 (예: 인공지능이란 무엇일까?) — 목차·차시 화면에 표시
);
...
ALTER TABLE lessons ADD COLUMN IF NOT EXISTS topic TEXT NOT NULL DEFAULT '';
...
// 부트스트랩 지문: SCHEMA·MIGRATIONS·CURRICULUM 본문이 바뀌면 값이 달라져 다음 콜드스타트에 전체 초기화가 한 번 다시 돈다.
// (node:crypto 없이 FNV-1a — 테스트 하네스가 require를 제한하므로 의존성을 늘리지 않는다)
function fingerprint(text) {
  let h = 0x811c9dc5;
  for (let i = 0; i < text.length; i++) { h ^= text.charCodeAt(i); h = Math.imul(h, 0x01000193) >>> 0; }
  return `${text.length}-${h.toString(16)}`;
}
const BOOTSTRAP_KEY = 'sys:bootstrap';
const BOOTSTRAP_MARK = fingerprint(SCHEMA + MIGRATIONS + JSON.stringify(CURRICULUM || []));

// 왕복 1번으로 "이미 초기화된 DB인지" 확인한다. 서버리스는 콜드스타트마다 init()을 다시 부르므로,
// 매번 DDL·시드 10여 번을 돌리면 DB가 멀 때 첫 요청이 수 초씩 걸린다.
// 관리자 계정이 사라진 경우도 전체 초기화로 보내 재시드한다. 테이블이 아직 없는 새 DB는 조회가 실패 → false.
async function bootstrapDone() {
  try {
    const row = await one(
      "SELECT (SELECT value FROM settings WHERE key = $1) AS mark, (SELECT count(*) FROM users WHERE role = 'admin')::int AS admins",
      [BOOTSTRAP_KEY]);
    return !!row && row.mark === BOOTSTRAP_MARK && row.admins > 0;
  } catch { return false; }
}

async function init() {
  if (await bootstrapDone()) return;
  ...
  try {
    ...
    seeded = false; // 지문을 남기지 않아 다음 콜드스타트에 다시 시도한다
    console.error('교육과정 시드 실패(무시하고 계속):', e.message);
  }
  if (seeded) {
    await q('INSERT INTO settings (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value', [BOOTSTRAP_KEY, BOOTSTRAP_MARK]);
  }
}

// 전환 모드(ACADEMY_ID)용 교육과정 지문. 스키마·관리자 시드는 monster-moakit 쪽이 이미 해 두었으므로
// 건드리지 않고, 레포로 관리하는 교육과정(프로그램·차시·교안 링크)만 붙인다.
const CURRICULUM_KEY = 'sys:curriculum';
const CURRICULUM_MARK = fingerprint(JSON.stringify(CURRICULUM || []));

// 지문이 같으면 조회 1번으로 끝난다. lib/curriculum.js 를 고치면 지문이 달라져 다음 콜드스타트에 한 번 돈다.
// 실패해도 서버 기동은 막지 않는다(지문을 남기지 않아 다음 콜드스타트에 다시 시도).
async function seedCurriculumOnly() {
  try {
    const row = await one('SELECT value FROM settings WHERE key = $1', [CURRICULUM_KEY]);
    if (row && row.value === CURRICULUM_MARK) return;
    await seedCurriculum({ q, one, academy: true });
    // 전환 모드: settings 는 hub 뷰 → ON CONFLICT 불가. 뷰 INSTEAD OF INSERT 트리거가 upsert 처리.
    await q('INSERT INTO settings (key, value) VALUES ($1, $2)', [CURRICULUM_KEY, CURRICULUM_MARK]);
  } catch (e) {
    console.error('교육과정 시드 실패(무시하고 계속):', e.message);
  }
}
...
function ready() {
  ...
  // 앱의 자동 스키마/시드 부트스트랩을 실행하면 public 테이블을 다시 만들어 충돌 → 교육과정 시드만 돌린다.
  if (ACADEMY_ID) { readyPromise ||= seedCurriculumOnly(); return readyPromise; }
...
async function getSettings() {
  const out = {};
  for (const row of await q("SELECT key, value FROM settings WHERE left(key,12) <> 'course_plan:' AND left(key,3) <> 'sb:' AND left(key,4) <> 'sys:' AND left(key,3) <> 'ws:'")) {
...
module.exports = { q, one, transaction, ready, log, getSettings, setSetting, clientIp, TS, ACADEMY_ID };
