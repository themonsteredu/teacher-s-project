...
async function ensureCurriculumLinks({ q, one }) {
  for (const c of CURRICULUM) {
    ...
    for (const ls of c.lessons) {
      // 차시(레슨) 찾기 — 없으면 만든다(커리큘럼에 새 차시를 추가하면 배포 시 자동 생성).
      let lesson = await one(
        'SELECT id FROM lessons WHERE program_id = $1 AND title = $2',
        [prog.id, ls.title]
      );
      if (!lesson) {
        const lmx = await one('SELECT COALESCE(MAX(position), -1) AS m FROM lessons WHERE program_id = $1', [prog.id]);
        lesson = await one(
          'INSERT INTO lessons (program_id, position, title) VALUES ($1, $2, $3) RETURNING id',
          [prog.id, (lmx ? Number(lmx.m) : -1) + 1, ls.title]
        );
        console.log(`차시 보충: ${c.title} / ${ls.title} 생성`);
      }
      for (const l of ls.links) {
        const has = await one(
          'SELECT 1 FROM program_links WHERE program_id = $1 AND url = $2',
          [prog.id, l.url]
        );
        if (has) continue; // 이미 연결됨
        const mx = await one(
          'SELECT COALESCE(MAX(position), -1) AS m FROM program_links WHERE program_id = $1',
          [prog.id]
        );
        const nextPos = (mx ? Number(mx.m) : -1) + 1;
        await q(
          'INSERT INTO program_links (program_id, lesson_id, position, kind, label, url) VALUES ($1, $2, $3, $4, $5, $6)',
          [prog.id, lesson.id, nextPos, 'aiapp', l.label, l.url]
        );
