...
async function handleCareerLogIngest(req, res, body) {
  ...
  const links = await q('SELECT label, url FROM program_links WHERE program_id = $1', [board.program_id]);
  ...
  if (!sourceEventId.startsWith(integration.sourceEventPrefix)) return send(res, 400, { error: 'invalid_source_event_id' }, origin);
