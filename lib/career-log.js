...
async function handleCareerLogIngest(req, res, body, {authorizeScience} = {}) {
  ...
  const science = integration.programRef === 'science-observation-ai-03';
  if (science && typeof authorizeScience !== 'function') return send(res,403,{error:'science_board_session_required'},origin);
  ...
  let studentId, scienceSessionId;
  try {
    if (science) {
      // The normal board session and its page-specific draftScope are mandatory for the same-origin activity.
      const who = await authorizeScience(body);
      studentId = who.careerStudentId;
      scienceSessionId = who.activitySessionId;
    } else {
  ...
  const assigned = links.some(link => {
    if (!science) return integration.linkMatches(link);
    try {
      const target = new URL(link.url,origin);
      return target.origin === origin && /3차시-학생용-감각짝맞추기\.html$/i.test(decodeURIComponent(target.pathname));
    } catch { return false; }
  });
  if (!assigned) return send(res, 403, { error: 'career_program_not_assigned' }, origin);
  ...
  if (science) {
    const observation = rawData?.observation;
    const plant = cleanText(observation?.plant_name,200), features = cleanText(observation?.features,1000);
    const eventToken = sourceEventId.slice(`${integration.sourceEventPrefix}${boardCode}:`.length);
    if (!sourceEventId.startsWith(`${integration.sourceEventPrefix}${boardCode}:`) || !validUuid(eventToken)
      || rawData.lesson !== 3 || rawData.activity !== 'plant-observation'
      || typeof observation?.plant_name !== 'string' || typeof observation?.features !== 'string'
      || !plant || !features || observation.plant_name !== plant || observation.features !== features
      || activityProcess !== `${plant} 식물을 관찰하고 눈에 보이는 특징을 기록함`
      || artifact !== `식물 관찰 카드 · ${plant}: ${features}` || reflection !== null) {
      return send(res,400,{error:'invalid_observation'},origin);
    }
    rawData = {lesson:3,activity:'plant-observation',observation:{plant_name:plant,features}};
  }
  try {
  ...
  const record = {
    ...
    raw_data: {
      ...
      hub: {
        ...
        ...(science ? {session_id:scienceSessionId} : {}),
      },
    },
  ...
  if (science && upstream.ok) {
    if (result.ok !== true || !validUuid(result.record_id) || result.student_id !== studentId || typeof result.duplicate !== 'boolean') {
      return send(res,502,{error:'invalid_edge_response'},origin);
    }
    // Receipt proves storage without exposing the private Career UUID to the activity window.
    return send(res,upstream.status,{ok:true,record_id:result.record_id,duplicate:result.duplicate,source_event_id:sourceEventId,draftScope:body.draftScope},origin);
  }
  if (science) return send(res,upstream.status,{error:typeof result.error === 'string'?result.error:'career_log_store_failed'},origin);
