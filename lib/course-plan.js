'use strict';
// Authoring metadata never grants app access. Submission record settings are consumed only by the authorized server submission service.
...
function issues(plan) {
  ...
  for (const v of plan.variants) v.sessions.forEach((s, i) => {
    ...
    if (s.record.mode === 'submission' && !s.submissions.enabled) out.push(`${label}: 진로기록을 남기려면 학생 제출 받기를 켜세요.`);
