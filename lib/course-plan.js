...
function submissionSettings(input) {
  const s = input || { enabled: true, types: ['photo', 'document', 'text'], sharing: 'teacher' };
  if (typeof s.enabled !== 'boolean' || !Array.isArray(s.types) || !s.types.length || s.types.some(t => !['photo', 'document', 'text'].includes(t)) || !['teacher', 'class'].includes(s.sharing)) fail('학생 제출 설정을 확인하세요.');
  return { enabled: s.enabled, types: [...new Set(s.types)], sharing: s.sharing };
}
function normalize(input, { lessonIds = [], fileIds = [] } = {}) {
  ...
  const variants = input.variants.map(v => {
    ...
    return { id, name, sessions: v.sessions.map(s => {
      ...
      return { title: text(s.title, 120), minutes: s.minutes, sourceLessons: [...new Set(s.sourceLessons.map(String))], bridge: text(s.bridge || ''), assets, submissions: submissionSettings(s.submissions),
  ...
  return { version: 1, appUrl: safeUrl(input.appUrl || ''), variants,
    ...(Object.prototype.hasOwnProperty.call(input, 'curriculum') ? { curriculum: curriculum.normalize(input.curriculum) } : {}) };
}
function issues(plan) {
  ...
  for (const v of plan.variants) v.sessions.forEach((s, i) => {
    ...
    if (!Object.values(s.assets).some(a => a.url || a.fileId) && !s.submissions.enabled) out.push(`${label}: 자료를 연결하거나 학생 제출 받기를 켜세요.`);
    if (s.record.mode === 'submission' && !s.submissions.enabled) out.push(`${label}: 진로기록을 설계하려면 학생 제출 받기를 켜세요.`);
...
module.exports = { normalize, issues, safeUrl, submissionSettings };
