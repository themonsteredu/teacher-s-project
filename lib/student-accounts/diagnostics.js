...
function databaseTarget(connectionString, databasePassword) {
  if (!connectionString) return { kind: 'missing' };
  try {
    const params = new Client(accountConnectionOptions({ databaseUrl: connectionString, databasePassword })).connectionParameters;
