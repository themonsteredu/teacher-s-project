...
const { accountConnectionOptions } = require('./connection');
const { createIdentityHelpers } = require('./identity');
...
function getService(config) {
  ...
  if (!service) service=createService(new Pool(accountConnectionOptions(config)),config.issuer);
  return service;
}
const identity=createIdentityHelpers({getConfig:accountConfig,getService});
async function handle(req,res,path,body,{getSessionUser,getSettings,findTeacher}) {
  const send=(status,data)=>{res.writeHead(status,{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store'});res.end(JSON.stringify(data));};
  const config=accountConfig();
  let stage='configuration';
  try {
    if (path==='/api/student-accounts/status' && req.method==='GET') return send(200,{enabled:config.enabled});
    ...
    if (path==='/api/student-accounts/records' && req.method==='GET') {
      stage='account_records';
      const query=new URL(req.url,'http://internal').searchParams;
      const limit=query.has('limit')?Number(query.get('limit')):30;
      const before=query.has('before')?query.get('before'):undefined;
      return send(200,await identity.readRecords(req,{limit,before}));
    }
    ...
    const access=/^\/api\/student-accounts\/schools\/([0-9a-f-]+)\/access$/.exec(path);
    if (access) {
      // 다른 모아킷 제품(모아랩)에 학교 열기·닫기. 관리자 전용, 같은 출처 요청만.
      if (req.method!=='PATCH') return send(405,{error:'허용되지 않은 요청입니다.'});
      await api.setAccess(user,access[1],String(body?.issuer||''),body?.enabled===true);return send(200,{ok:true});
    }
    ...
    if (!e.status) console.error('student_account_request_failed', {
      ...
      stage,
      accountTarget:databaseTarget(config.databaseUrl,config.databasePassword),
      credentialSource:config.databasePassword===undefined ? 'connection_url' : 'separate_password',
    });
    return send(e.status||500,{error:e.status?e.message:'학생 계정 처리 중 오류가 발생했습니다.'});
  }
}
module.exports={handle,configured,...identity};
