'use strict';
const crypto = require('node:crypto');
const S = require('./security');
const R = require('./roster');
// 학교를 열어 줄 수 있는 모아킷 제품. 열린 제품의 관리자는 담당자 지정 없이 학교를 관리한다 (db/moakit-accounts-0002-school-access.sql).
const ISSUERS = ['moakit-hub', 'moakit-lab'];
// All SQL runs against the explicitly configured central account pool.
// Transaction boundaries include authorization, account mutation and audit.
function createService(pool, issuer) {
  async function tx(fn) {
    const c = await pool.connect();
    try { await c.query('BEGIN'); const result = await fn(c); await c.query('COMMIT'); return result; }
    catch (e) { await c.query('ROLLBACK').catch(() => {}); throw e; }
    finally { c.release(); }
  }
  const actor = user => `${issuer}:${user.id}`;
  async function authorize(c, user, school) {
    if (!user || !['teacher','admin'].includes(user.role) || !S.uuid(school)) S.fail(403, '학교 관리 권한이 없습니다.');
    // 이 제품의 관리자는 모든 학교를 관리한다 (담당자 지정·다른 제품의 열기 여부와 무관).
    // 존재 확인은 남긴다 — 건너뛰면 없는 학교 UUID 가 403 대신 나중에 알 수 없는 오류로 터진다.
    if (user.role === 'admin') {
      const exists = await c.query('SELECT 1 FROM moakit_accounts.schools WHERE id=$1 FOR SHARE', [school]);
      if (!exists.rowCount) S.fail(404, '학교를 찾을 수 없습니다.');
      return;
    }
    const r = await c.query('SELECT 1 FROM moakit_accounts.managers WHERE school_id=$1 AND issuer=$2 AND teacher_id=$3 FOR SHARE', [school, issuer, String(user.id)]);
    if (r.rowCount) return;
    // 다른 제품(모아랩)에서 이 제품에 열어 준 학교는 관리자가 담당자 지정 없이 관리한다.
    if (user.role === 'admin') {
      const open = await c.query('SELECT 1 FROM moakit_accounts.school_access WHERE school_id=$1 AND issuer=$2 FOR SHARE', [school, issuer]);
      if (open.rowCount) return;
    }
    S.fail(403, '학교 관리 권한이 없습니다.');
  }
  const audit = (c, user, school, action, account = null) => c.query('INSERT INTO moakit_accounts.audit(school_id,actor,action,account_id) VALUES($1,$2,$3,$4)', [school, actor(user), action, account]);
  async function session(c, token, lock = false) {
    if (!/^[a-f0-9]{64}$/.test(token || '')) S.fail(401, '학생 로그인이 필요합니다.');
    const r = await c.query(`SELECT a.id,a.career_student_id,a.username,a.must_change_password,s.expires_at FROM moakit_accounts.sessions s JOIN moakit_accounts.accounts a ON a.id=s.account_id WHERE s.token_hash=$1 AND s.expires_at>now() AND a.active=true${lock ? ' FOR UPDATE OF a,s' : ''}`, [S.hash(token)]);
    if (!r.rowCount) S.fail(401, '학생 로그인이 필요합니다.');
    return r.rows[0];
  }
  function identity(a, token) {
    // Account IDs, display names and school IDs never become Career identities.
    if (!S.uuid(a.id) || !S.uuid(a.career_student_id)) S.fail(401,'학생 계정을 확인해 주세요.');
    return {accountId:a.id,careerStudentId:a.career_student_id,sessionHash:S.hash(token),expiresAt:a.expires_at,mustChangePassword:a.must_change_password===true};
  }
  function ready(a) {
    if (a.must_change_password) S.fail(403,'처음 받은 비밀번호를 변경한 뒤 수업에 참여하세요.');
  }
  return {
    // These methods are server-only. A caller-supplied UUID cannot select an account.
    async resolveStudent(token) { return tx(async c=>identity(await session(c,token),token)); },
    async authorizeStudent(token, school) {
      if (!S.uuid(school)) S.fail(403,'수업에 연결된 학교를 확인해 주세요.');
      return tx(async c=>{
        const a=await session(c,token);ready(a);
        const r=await c.query('SELECT 1 FROM moakit_accounts.memberships WHERE account_id=$1 AND school_id=$2 AND active=true FOR SHARE',[a.id,school]);
        if (!r.rowCount) S.fail(403,'이 학교 수업에 참여할 수 있는 학생 계정이 필요합니다.');
        return identity(a,token);
      });
    },
    async authorizeSchool(user, school) { return tx(async c=>{await authorize(c,user,school);return {schoolId:school};}); },
    async readRecords(token, {limit=30,before}={}) {
      if (!Number.isInteger(limit) || limit<1 || limit>100 || (before!==undefined && !S.uuid(before))) S.fail(400,'기록 조회 범위를 확인해 주세요.');
      return tx(async c=>{
        const a=await session(c,token);ready(a);
        const own=identity(a,token);
        // 담당자가 정정한 기록은 최신 버전만 보여준다 (원본은 supersedes_id 로 이어진 이력, 모아랩 학생 기록 열람과 같은 규칙).
        const r=await c.query(`SELECT id,student_id,session_ref,program_ref,occurred_at,process,artifact,reflection,source,verification_status,verified_by,verified_at,raw_data,source_event_id,supersedes_id FROM career_log.records r WHERE student_id=$1 AND NOT EXISTS (SELECT 1 FROM career_log.records n WHERE n.student_id=r.student_id AND n.supersedes_id=r.id)${before ? ' AND (occurred_at,id)<(SELECT occurred_at,id FROM career_log.records WHERE id=$3 AND student_id=$1)' : ''} ORDER BY occurred_at DESC,id DESC LIMIT $2`,before?[own.careerStudentId,limit+1,before]:[own.careerStudentId,limit+1]);
        const records=r.rows.slice(0,limit);
        return {username:a.username,records,nextBefore:r.rows.length>limit?records.at(-1).id:null};
      });
    },
    async schools(user) {
      // 담당 학교 + (관리자라면) 모든 학교. openedTo 는 이 학교가 열린 다른 제품 목록.
      return tx(async c => (await c.query(`SELECT s.id, s.name, bool_or(m.teacher_id IS NOT NULL) AS manager,
          bool_or(a.issuer=$1) AS opened,
          COALESCE(array_agg(DISTINCT a.issuer) FILTER (WHERE a.issuer IS NOT NULL), '{}') AS opened_to
        FROM moakit_accounts.schools s
        LEFT JOIN moakit_accounts.managers m ON m.school_id=s.id AND m.issuer=$1 AND m.teacher_id=$2
        LEFT JOIN moakit_accounts.school_access a ON a.school_id=s.id
        GROUP BY s.id, s.name
        HAVING bool_or(m.teacher_id IS NOT NULL) OR $3::boolean
        ORDER BY s.name`, [issuer, String(user.id), user?.role === 'admin'])).rows
        // via: manager=담당자 지정 · open=다른 제품이 열어 줌 · admin=관리자 권한으로만 보이는 학교
        .map(row => ({ id: row.id, name: row.name, via: row.manager ? 'manager' : row.opened ? 'open' : 'admin', openedTo: (row.opened_to || []).filter(value => value !== issuer) })));
    },
    async setAccess(user, school, target, enabled) {
      if (user?.role !== 'admin') S.fail(403, '관리자만 다른 제품에 학교를 열 수 있습니다.');
      if (!ISSUERS.includes(target) || target === issuer) S.fail(400, '열어 줄 제품을 확인하세요.');
      return tx(async c => {
        await authorize(c, user, school);
        if (enabled) await c.query('INSERT INTO moakit_accounts.school_access(school_id,issuer,opened_by) VALUES($1,$2,$3) ON CONFLICT DO NOTHING', [school, target, actor(user)]);
        else await c.query('DELETE FROM moakit_accounts.school_access WHERE school_id=$1 AND issuer=$2', [school, target]);
        await audit(c, user, school, `${enabled ? 'access_opened' : 'access_closed'}:${target}`);
      });
    },
    async createSchool(user, name) {
      if (user?.role !== 'admin') S.fail(403, '관리자만 학교를 등록할 수 있습니다.');
      name = S.text(name, 120);
      return tx(async c => {
        const school = (await c.query('INSERT INTO moakit_accounts.schools(name) VALUES($1) RETURNING id,name', [name])).rows[0];
        await c.query('INSERT INTO moakit_accounts.managers VALUES($1,$2,$3)', [school.id, issuer, String(user.id)]);
        await audit(c, user, school.id, 'school_created'); return school;
      });
    },
    async grantManager(user, school, teacherId) {
      if (user?.role !== 'admin') S.fail(403, '관리자만 담당자를 지정할 수 있습니다.');
      return tx(async c => { await authorize(c,user,school); await c.query('INSERT INTO moakit_accounts.managers VALUES($1,$2,$3) ON CONFLICT DO NOTHING', [school,issuer,String(teacherId)]); await audit(c,user,school,'manager_granted'); });
    },
    async list(user, school) {
      return tx(async c => {
        await authorize(c,user,school);
        const result=await c.query('SELECT a.id,a.username,a.active,a.must_change_password,m.display_name,m.class_name FROM moakit_accounts.memberships m JOIN moakit_accounts.accounts a ON a.id=m.account_id WHERE m.school_id=$1 AND m.active=true ORDER BY m.class_name,m.display_name', [school]);
        return result.rows.map(row=>({...row,...R.fromLabel(row.class_name)})).sort(R.compare);
      });
    },
    async provision(user, school, input) {
      const rows = S.roster(input);
      return tx(async c => {
        await authorize(c,user,school);
        await R.checkAvailable(c,school,rows);
        const result = [];
        for (const row of rows) {
          const studentId = crypto.randomUUID();
          const username = 'm' + crypto.randomBytes(10).toString('hex');
          const temporaryPassword = crypto.randomBytes(12).toString('base64url');
          await c.query('INSERT INTO career_log.students(id) VALUES($1)', [studentId]);
          const a = (await c.query('INSERT INTO moakit_accounts.accounts(career_student_id,username,password_hash) VALUES($1,$2,$3) RETURNING id', [studentId,username,await S.passwordHash(temporaryPassword)])).rows[0];
          await c.query('INSERT INTO moakit_accounts.memberships(account_id,school_id,display_name,class_name) VALUES($1,$2,$3,$4)', [a.id,school,row.displayName,row.className]);
          await audit(c,user,school,'account_issued',a.id);
          result.push({ id: a.id, username, temporaryPassword, ...row });
        }
        return result;
      });
    },
    async updateMember(user, school, id, input) {
      if (!S.uuid(id)) S.fail(400,'학생을 확인해 주세요.');
      const row=R.normalize(input);
      return tx(async c => {
        await authorize(c,user,school);
        await R.checkAvailable(c,school,[row],id);
        const r=await c.query('UPDATE moakit_accounts.memberships SET display_name=$1,class_name=$2 WHERE account_id=$3 AND school_id=$4 AND active=true RETURNING account_id',[row.displayName,row.className,id,school]);
        if (!r.rowCount) S.fail(404,'학생을 찾을 수 없습니다.');
        await audit(c,user,school,'membership_updated',id);
      });
    },
    async reset(user, school, id) {
      if (!S.uuid(id)) S.fail(400,'학생을 확인해 주세요.');
      return tx(async c => {
        await authorize(c,user,school);
        const r=await c.query('SELECT a.id FROM moakit_accounts.accounts a JOIN moakit_accounts.memberships m ON m.account_id=a.id WHERE a.id=$1 AND m.school_id=$2 AND m.active=true FOR UPDATE OF a',[id,school]);
        if (!r.rowCount) S.fail(404,'학생을 찾을 수 없습니다.');
        const temporaryPassword=crypto.randomBytes(12).toString('base64url');
        await c.query('UPDATE moakit_accounts.accounts SET password_hash=$1,must_change_password=true WHERE id=$2',[await S.passwordHash(temporaryPassword),id]);
        await c.query('DELETE FROM moakit_accounts.sessions WHERE account_id=$1',[id]);
        await audit(c,user,school,'password_reset',id); return { temporaryPassword };
      });
    },
    async login(username, password, clientKey) {
      if (typeof username!=='string' || !/^m[a-f0-9]{20}$/.test(username) || typeof password!=='string' || password.length>128) S.fail(401,'아이디 또는 비밀번호가 올바르지 않습니다.');
      // Persist limits in a separate transaction so failed logins do not roll back the counter.
      await tx(async c => {
        for (const [key, limit] of [[S.hash('user:'+username),20],[S.hash('client:'+clientKey),500]]) {
          const r=await c.query("INSERT INTO moakit_accounts.login_limits VALUES($1,1,now()+interval '15 minutes') ON CONFLICT(key_hash) DO UPDATE SET attempts=CASE WHEN moakit_accounts.login_limits.resets_at<now() THEN 1 ELSE moakit_accounts.login_limits.attempts+1 END,resets_at=CASE WHEN moakit_accounts.login_limits.resets_at<now() THEN now()+interval '15 minutes' ELSE moakit_accounts.login_limits.resets_at END RETURNING attempts",[key]);
          if (r.rows[0].attempts>limit) S.fail(429,'로그인 시도가 많습니다. 15분 후 다시 시도하세요.');
        }
      });
      return tx(async c => {
        const a=(await c.query('SELECT * FROM moakit_accounts.accounts WHERE username=$1 FOR UPDATE',[username])).rows[0];
        if (!a || !a.active || !await S.passwordMatches(password,a.password_hash)) S.fail(401,'아이디 또는 비밀번호가 올바르지 않습니다.');
        const token=crypto.randomBytes(32).toString('hex');
        await c.query("INSERT INTO moakit_accounts.sessions VALUES($1,$2,now()+interval '8 hours')",[S.hash(token),a.id]);
        return { token, mustChangePassword:a.must_change_password };
      });
    },
    async me(token) { return tx(c=>session(c,token)); },
    async logout(token) { if (/^[a-f0-9]{64}$/.test(token||'')) await tx(c=>c.query('DELETE FROM moakit_accounts.sessions WHERE token_hash=$1',[S.hash(token)])); },
    async changePassword(token, current, next) {
      S.passwordInput(next);
      if (current===next) S.fail(400,'새 비밀번호를 다르게 입력하세요.');
      return tx(async c=>{
        const a=await session(c,token,true);
        const stored=(await c.query('SELECT password_hash FROM moakit_accounts.accounts WHERE id=$1',[a.id])).rows[0];
        if (!await S.passwordMatches(current,stored.password_hash)) S.fail(401,'현재 비밀번호를 확인하세요.');
        await c.query('UPDATE moakit_accounts.accounts SET password_hash=$1,must_change_password=false WHERE id=$2',[await S.passwordHash(next),a.id]);
        await c.query('DELETE FROM moakit_accounts.sessions WHERE account_id=$1',[a.id]);
        // Reauthentication rotates the session and clears all shared-device sessions.
      });
    },
  };
}
module.exports={createService};
