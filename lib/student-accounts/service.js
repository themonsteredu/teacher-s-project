  ...
  return {
    ...
    async readRecords(token, {limit=30,before}={}) {
      ...
      return tx(async c=>{
        ...
        const r=await c.query(`SELECT id,student_id,session_ref,program_ref,occurred_at,process,artifact,reflection,source,verification_status,verified_by,verified_at,raw_data,source_event_id,supersedes_id FROM career_log.records WHERE student_id=$1${before ? ' AND (occurred_at,id)<(SELECT occurred_at,id FROM career_log.records WHERE id=$3 AND student_id=$1)' : ''} ORDER BY occurred_at DESC,id DESC LIMIT $2`,before?[own.careerStudentId,limit+1,before]:[own.careerStudentId,limit+1]);
