...
const DRONE_ORIGINS = [
  /^https:\/\/drone-six-smoky\.vercel\.app$/,
  /^https:\/\/drone-[a-z0-9-]+-themonsteredu\.vercel\.app$/,
];

const INTEGRATIONS = {
  'aviation-mobility-01': {
    programRef: 'aviation-mobility-01',
    origins: DRONE_ORIGINS,
    sourceEventPrefix: 'aviation-mobility-01:',
    linkMatches: (link) => {
      try { return DRONE_ORIGINS.some((pattern) => pattern.test(new URL(link.url).origin)); }
      catch { return false; }
    },
  },
  'history-ai-01': {
