...
function createService({ db, storage, openBoardByCode, sharedMaterials, parseCookies, secure = false, clientIp, deliverCareer = career.deliver }) {
  ...
  async function session(req, res, b, create = false) {
    ...
    if (current && current.expires > Date.now()) {
      if (!UUID.test(current.careerStudentId || '')) {
        await db.transaction(async c => {
          await lock(c,`sb:session:${b.id}:${owner}`);
          const latest = parse(await c.one('SELECT value FROM settings WHERE key=$1 FOR UPDATE',[`sb:session:${b.id}:${owner}`]));
          if (!latest || latest.expires <= Date.now()) fail(401,'학생 입장을 다시 확인하세요.');
          latest.careerStudentId ||= crypto.randomUUID();
          await put(`sb:session:${b.id}:${owner}`,latest,c);
          current.careerStudentId = latest.careerStudentId;
        });
      }
      return {owner,expires:current.expires,careerStudentId:current.careerStudentId};
    }
    ...
    const careerStudentId = crypto.randomUUID();
    await put(`sb:session:${b.id}:${nextOwner}`, { expires, careerStudentId });
    cookie(res,b.id,next);
    return { owner: nextOwner, expires, careerStudentId };
  }
  async function identity(req,res,code) {
    const {board:b} = await board(code);
    return (await session(req,res,b,true)).careerStudentId;
  }
  ...
  async function studentCourse(b, cfg) {
    ...
    return { name:cfg.name, activeSession:cfg.activeSession, sessions:cfg.sessions.map(s => {
      ...
      return { id:s.id, title:s.title, minutes:s.minutes, bridge:s.bridge, submissions:s.submissions, record:{enabled:s.record?.mode === 'submission',completion:s.record?.completion || ''}, materials:{
  ...
  async function list(req, res, code) {
    ...
    const posts = await Promise.all(rows.slice(0,60).map(async p => {
      ...
      return { id:p.id, student_name:p.student_name, title:m?.title || '활동 제출물', content:p.content, created_at:p.created_at, file_name:p.file_name, mime:p.mime, size:p.size,
        ...
        visibility:!p.hidden && (!m || publicIds.includes(m.sessionId)) ? 'class' : 'teacher', careerStatus:career.receipt(m).state, career: m?.owner === who.owner ? career.receipt(m) : undefined,
  ...
  async function submit(req,res,code,body) {
    ...
    const result = await db.transaction(async c => {
      ...
      if (s.record?.mode === 'submission') {
        if (!s.record.process?.trim() || !s.record.completion?.trim() || !s.record.original?.trim()) fail(400,'선생님이 차시의 기록 설정을 확인해야 합니다.');
        const identity = JSON.parse(currentSession.value);
        // Never adopt an unsigned browser UUID, name, board code or submitted body ID.
        if (!UUID.test(identity.careerStudentId || '')) {
          identity.careerStudentId = crypto.randomUUID();
          await put(`sb:session:${b.id}:${who.owner}`, identity, c);
        }
        const attachment = upload ? await storage.archiveSubmission(upload).catch(() => fail(502,'첨부 원본 보관에 실패했습니다. 같은 제출로 다시 시도하세요.')) : null;
        careerSnapshot = career.snapshot({studentId:identity.careerStudentId,board:b,session:s,data,attachment});
      }
      ...
      await put(`sb:post:${row.id}`,{owner:who.owner,studentName:data.name,title:data.title,sessionId:s.id,sessionTitle:s.title,requestId:data.requestId,...(careerSnapshot ? {career:{state:'pending',payload:careerSnapshot}} : {})},c);
    ...
    return {...response,career:careerRequested ? await saveCareer(req,res,code,result.id) : {state:'not_requested'}};
  }
  async function saveCareer(req,res,code,id) {
    sameOrigin(req);
    const {board:b} = await board(code), who = await session(req,res,b);
    ...
    return db.transaction(async c => {
      ...
      if (!m || m.owner !== who.owner) fail(403,'내 제출물의 기록만 저장할 수 있습니다.');
      if (!m.career || m.career.state === 'saved') return career.receipt(m);
      ...
      if (!identity || identity.expires <= Date.now() || identity.careerStudentId !== m.career.payload.student_id) fail(401,'학생 입장을 다시 확인하세요.');
      try {
  ...
  async function records(req,res,code) {
    ...
    if (before && !/^[1-9]\d{0,18}$/.test(before)) fail(400,'목록 위치를 확인하세요.');
    const rows = await db.q("WITH career_posts AS MATERIALIZED (SELECT key,value FROM settings WHERE key ~ '^sb:post:[1-9][0-9]*$') SELECT key,value FROM career_posts WHERE value::jsonb->>'owner'=$1 AND value::jsonb->'career'->'payload'->>'session_ref'=$2 AND ($3::bigint IS NULL OR substring(key from 9)::bigint<$3::bigint) ORDER BY substring(key from 9)::bigint DESC LIMIT 61",[who.owner,`hub-board:${b.id}`,before]);
    return {scope:'current_student_session',records:rows.slice(0,60).map(row => {const m=JSON.parse(row.value);return {postId:row.key.slice(8),...career.receipt(m,true)};}),next:rows.length>60?rows[59].key.slice(8):null};
  }
  ...
  async function recordFile(req,res,code,id) {
    ...
    if (!m || m.owner !== who.owner || m.career?.payload.session_ref !== `hub-board:${b.id}`) fail(403,'내 기록만 열람할 수 있습니다.');
  ...
  return {list,sign,submit,file,leave,settings,saveSettings,moderate,teacherPosts,saveCareer,records,recordFile,identity,teacherRecords};
}
