...
// 활동지는 여러 장으로 찍힌다. 한 제출에 이만큼까지 붙일 수 있다.
const MAX_FILES = 5;
const hash = s => crypto.createHash('sha256').update(s).digest('hex');
// settings 값에 JSON 이 아닌 것(빈 문자열 등)이 섞여 있어도 화면 전체가 죽지 않게 한다.
// 실제로 sb:post: 행 하나가 빈 문자열이라 학생 목록 조회가 22P02 로 터졌다. 읽는 쪽은 없는 값으로 취급한다.
const parseJson = text => { try { return typeof text === 'string' && text.trimStart().startsWith('{') ? JSON.parse(text) : null; } catch { return null; } };
// 같은 규칙의 SQL 판: 중괄호로 시작하지 않으면 캐스팅하지 않는다(PostgreSQL 에는 안전한 형변환이 없다).
const JSON_META = column => `CASE WHEN left(btrim(${column}),1)='{' THEN (${column})::jsonb END`;
const fail = (status, message) => { const e = new Error(message); e.status = status; throw e; };
const isImage = mime => typeof mime === 'string' && mime.startsWith('image/');
// 첨부 목록의 원본은 sb:post 메타다. 이 기능 이전의 제출은 board_posts 한 칸에만
// 있으므로 거기서 한 장짜리 목록을 만들어 준다 — 옛 제출도 같은 코드로 다룬다.
function attachmentsOf(meta, post) {
  const saved = Array.isArray(meta?.attachments) ? meta.attachments.filter(a => a?.path) : [];
  if (saved.length) return saved;
  if (!post?.storage_path) return [];
  // 옛 행은 mime 칸이 비어 있기도 하다. 그때는 파일 이름의 확장자로 본다.
  const mime = post.mime || MIME[String(post.file_name || '').split('.').pop().toLowerCase()] || null;
  return [{name:post.file_name,mime,size:post.size,path:post.storage_path}];
}
function pickAttachment(list, index) {
  const i = index === undefined || index === null || index === '' ? 0 : Number(index);
  if (!Number.isInteger(i) || i < 0 || i >= list.length) fail(404,'첨부 파일을 찾을 수 없습니다.');
  return list[i];
}
const parse = row => parseJson(row?.value);
...
function createService({ db, storage, openBoardByCode, sharedMaterials, parseCookies, secure = false, clientIp, deliverCareer = career.deliver, resolveStudent = async () => null, authorizeStudent = async () => fail(503,'학생 계정 연결을 확인하세요.'), authorizeSchool = async () => fail(503,'학생 계정 연결을 확인하세요.'), logoutStudent = async () => {}, careerAvailable = () => false }) {
  ...
  // 한 화면에 사진이 수십 장 나온다. 한 장씩 서명하면 목록이 그만큼 느려지므로
  // 화면에 쓸 사진을 모아 한 번에 받는다.
  const signImages = async (lists, ttl) => {
    const paths = lists.flat().filter(a => isImage(a.mime)).map(a => a.path);
    if (!storage.storageEnabled || !paths.length) return new Map();
    if (storage.createSignedDownloads) return storage.createSignedDownloads(paths,{ttl}).catch(() => new Map());
    const signed = new Map(); // 묶음 발급이 없는 저장소 어댑터
    for (const path of [...new Set(paths)]) {
      const url = await storage.createSignedDownload(path,{ttl}).catch(() => null);
      if (url) signed.set(path,url);
    }
    return signed;
  };
  // 화면에 내보내는 첨부 형태. 저장 경로는 절대 브라우저로 나가지 않는다.
  const previewList = (list, signed) => list.map((a, index) => ({
    index, name:a.name, mime:a.mime, size:a.size, previewUrl: signed.get(a.path) || null,
  }));
  ...
  async function ownerBoard(id, user) {
    ...
    // 관리자는 모든 수업을 진행한다 — lib/api.js 의 authorizeSchoolBoard() 와 같은 규칙이다.
    // 이 예외가 없으면 학교가 삭제됐거나 학생 계정 연결이 잠시 안 될 때(503) 관리자가
    // 남의 수업을 '지울 수는 있는데 열지는 못하는' 상태가 된다.
    if (cfg.career?.schoolId && user.role !== 'admin') await authorizeSchool(user,cfg.career.schoolId);
    return b;
  }
  ...
  // 학교가 연결돼 있는 동안에는 학생 계정 로그인이 필요하다. 진로기록 저장만 끈 상태(enabled:false)에서도
  // 유지되며, 코드만으로 입장시키려면 교사가 연결을 '해제'해야 한다(saveSettings 의 disconnect).
  async function accountFor(req,cfg) {
    const required = !!cfg.career?.schoolId;
    let account = null;
    try {
      account = required ? await authorizeStudent(req,cfg.career.schoolId) : await resolveStudent(req);
      ...
      // 계정이 필요 없는 수업(학교 미연결)에서는 기기에 남은 만료·불량 학생 세션 때문에 입장을 막지 않는다.
      // 공유 태블릿으로 참여 코드를 넣은 학생이 이유 없이 로그인 화면에 걸리던 원인이다.
      if (required) throw e;
      account = null;
    }
    if (required && !account) fail(401,'학생 계정으로 로그인하세요.');
    return account;
  }
  ...
  async function list(req, res, code) {
    ...
    // 메타(sb:post:)는 한 번만 jsonb 로 바꾸고, JSON 이 아닌 값은 없는 것으로 친다.
    // 실제로 빈 문자열 행 하나 때문에 차시 고르기·내 제출물 조회가 통째로 500(22P02)으로 죽었다.
    const rows = await db.q(`SELECT * FROM (
        SELECT p.*, m.value AS submission_meta, ${JSON_META('m.value')} AS meta FROM board_posts p
        LEFT JOIN settings m ON m.key = 'sb:post:' || p.id::text WHERE p.board_id=$1
      ) p
      WHERE ((p.hidden=false AND (p.meta IS NULL OR p.meta->>'sessionId'=ANY($6::text[]))) OR (p.meta->>'accountId'=$7 AND $7<>'') OR (p.meta->>'accountId' IS NULL AND $7='' AND p.meta->>'owner'=$2))
      AND ($3='' OR p.meta->>'sessionId'=$3) AND ($4=false OR (p.meta->>'accountId'=$7 AND $7<>'') OR (p.meta->>'accountId' IS NULL AND $7='' AND p.meta->>'owner'=$2))
    ...
    const metas = page.map(p => parseJson(p.submission_meta));
    const lists = page.map((p,i) => attachmentsOf(metas[i],p));
    const signed = await signImages(lists,60);
    const posts = page.map((p,i) => {
      const m = metas[i], attachments = previewList(lists[i],signed);
      return { id:p.id, student_name:p.student_name, title:m?.title || '활동 제출물', content:p.content, created_at:p.created_at, file_name:p.file_name, mime:p.mime, size:p.size,
        ...
        attachments, previewUrl:attachments.find(a => a.previewUrl)?.previewUrl || null };
    });
  ...
  async function submit(req,res,code,body) {
    ...
    // 예전 화면은 uploadId 하나만 보낸다. 둘 다 받아 한 목록으로 다룬다.
    const requested = Array.isArray(body?.uploadIds) ? body.uploadIds : body?.uploadId ? [body.uploadId] : [];
    const data = { requestId:body.requestId.toLowerCase(), sessionId:short(body.sessionId,80,true), title:short(body.title,120,true), name:short(body.student_name,20,true), content:short(body.content || '',2000), uploadIds:requested };
    if (requested.length > MAX_FILES) fail(400,`파일은 한 번에 ${MAX_FILES}개까지 올릴 수 있습니다.`);
    if (requested.some(id => !UUID.test(id || '')) || new Set(requested).size !== requested.length) fail(400,'첨부 파일을 다시 선택하세요.');
    if (!requested.length && !data.content) fail(400,'활동 내용을 쓰거나 파일을 첨부하세요.');
    ...
    const result = await db.transaction(async c => {
      ...
      if (!uploads.length) allowed(s,'text');
      ...
      if (cfg.career?.enabled === true && s.record?.mode === 'submission') {
        ...
        const attachments = [];
        for (const file of uploads) attachments.push(await storage.archiveSubmission(file).catch(() => fail(502,'첨부 원본 보관에 실패했습니다. 같은 제출로 다시 시도하세요.')));
        careerSnapshot = career.snapshot({studentId:account.careerStudentId,board:b,session:s,data,attachments});
      }
      ...
      await put(`sb:post:${row.id}`,{owner:who.owner,...(account ? {accountId:account.accountId,schoolId:cfg.career?.schoolId || null} : {}),studentName:data.name,title:data.title,sessionId:s.id,sessionTitle:s.title,requestId:data.requestId,
        ...(uploads.length ? {attachments:uploads.map(f => ({name:f.name,mime:f.mime,size:f.size,path:f.path}))} : {}),
        ...(careerSnapshot ? {career:{state:'pending',payload:careerSnapshot}} : {})},c);
  ...
  async function teacherRecords(id,user) {
    ...
    const rows = await db.q("WITH career_posts AS MATERIALIZED (SELECT key,value FROM settings WHERE key ~ '^sb:post:[1-9][0-9]*$' AND left(btrim(value),1)='{') SELECT key,value FROM career_posts WHERE value::jsonb->'career'->'payload'->>'session_ref'=$1 ORDER BY substring(key from 9)::bigint DESC LIMIT 200",[`hub-board:${b.id}`]);
  ...
  async function recordFile(req,res,code,id,index) {
    const {board:b} = await board(code), who = await session(req,res,b);
    const m = await get(`sb:post:${id}`), saved = m?.career?.payload?.raw_data?.submission;
    if (!m || !who.account || !owns(m,who) || m.career?.payload.session_ref !== `hub-board:${b.id}`) fail(403,'내 기록만 열람할 수 있습니다.');
    const list = (saved?.attachments?.length ? saved.attachments : [saved?.attachment]).filter(a => a?.storage_path?.startsWith('career-originals/'));
    if (!list.length) fail(404,'보관된 첨부가 없습니다.');
    const attachment = pickAttachment(list,index);
    return {url:await storage.createSignedDownload(attachment.storage_path,{ttl:60,downloadName:attachment.name})};
  }
  async function file(req,res,code,id,index) {
    ...
    if (!p) fail(404,'첨부 파일을 찾을 수 없습니다.');
    const meta = await get(`sb:post:${p.id}`);
    const list = attachmentsOf(meta,p);
    if (!list.length) fail(404,'첨부 파일을 찾을 수 없습니다.');
    ...
    if (!storage.storageEnabled) fail(503,'파일 저장소가 설정되지 않았습니다.');
    const attachment = pickAttachment(list,index);
    return { url:await storage.createSignedDownload(attachment.path,{ttl:60,downloadName:attachment.name}), name:attachment.name };
  }
  ...
  async function saveSettings(id,user,body) {
    ...
    return db.transaction(async c => {
      ...
      // 학교 연결 해제는 교사가 명시적으로 요청할 때만 한다(disconnect). 체크만 풀면 학교는 남는다 —
      // 조용히 공개 수업이 되지 않게 하려는 기존 규칙이고, 해제하면 참여 코드만 아는 사람도 제출할 수 있다.
      const disconnect = requestedCareer.disconnect === true;
      if (disconnect && (requestedCareer.enabled || requestedCareer.schoolId)) fail(400,'학생 계정 연결을 해제하려면 진로기록 연결을 먼저 끄세요.');
      if (disconnect && priorCareer.schoolId) await authorizeSchool(user,priorCareer.schoolId);
      const schoolId = disconnect ? null : (requestedCareer.schoolId || priorCareer.schoolId || null);
      if (priorCareer.schoolId && schoolId && schoolId !== priorCareer.schoolId) fail(409,'학교가 연결된 수업은 다른 학교로 변경할 수 없습니다. 새 수업을 만드세요.');
  ...
  async function teacherPosts(posts,user) {
    ...
    const metas = new Map(rows.map(r => [r.key,parseJson(r.value)]).filter(([,meta]) => meta));
    const configs = new Map();
    for (const id of new Set(posts.map(p => p.board_id))) {
      const cfg = await get(cfgKey(id));
      // ownerBoard() 와 같은 이유로 관리자는 학교 확인을 건너뛴다. 이게 없으면 학생이
      // 제출하기 시작한 뒤부터 관리자가 남의 수업 제출물을 못 열었다(제출 0건이면 위에서 끝나 증상이 늦게 나타난다).
      if (cfg?.career?.schoolId && user?.role !== 'admin') await authorizeSchool(user,cfg.career.schoolId);
      configs.set(String(id), cfg);
    }
    // 사진이 여러 장인 제출이라 목록에 전부 실어 준다. 첫 장은 기존 previewUrl 과 같다.
    const lists = posts.map(p => attachmentsOf(metas.get(`sb:post:${p.id}`),p));
    const signed = await signImages(lists,600);
    return posts.map((p,i) => {
      ...
      const attachments = previewList(lists[i],signed);
      return {...p,attachments,previewUrl:attachments.find(a => a.previewUrl)?.previewUrl || p.previewUrl || null,
        ...(m ? {hidden:p.hidden || !canShare,submission:true,title:m.title,sessionTitle:m.sessionTitle,studentKey:m.accountId || `name:${m.studentName || p.student_name}`,careerStatus:career.receipt(m).state,career:career.receipt(m)} : {})};
    });
  }
...
// 두 번째 장부터는 board_posts 가 아니라 sb:post 메타에만 경로가 있다. 제출물·수업·
// 프로그램을 지울 때 이것으로 함께 모아야 저장소에 사진이 남지 않는다.
// career-originals 는 여기서 다루지 않는다 — 저장된 진로기록이 가리키는 원본이라
// storage.removeObject 가 애초에 거부한다.
function storagePathsOf(metaValue, post) {
  let meta = null;
  try { meta = metaValue ? JSON.parse(metaValue) : null; } catch { meta = null; }
  return [...new Set(attachmentsOf(meta,post).map(a => a.path).filter(p => p && !p.startsWith('career-originals/')))];
}
module.exports = {createService,fileSpec,sameOrigin,attachmentsOf,storagePathsOf,MAX_FILES};
