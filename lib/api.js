...
route('POST', /^\/api\/programs\/(\d+)\/boards$/, 'teacher', async (req, res, ctx) => {
  ...
  if (wanted) {
    ...
    board = await one(
      ...
      [program.id, title, wanted, ctx.user.id]);
  } else {
...
route('GET', /^\/api\/my-boards$/, 'teacher', async (req, res, ctx) => {
  ...
  const rows = await q(
    `SELECT b.id, b.title, b.code, b.is_open, b.program_id, ${TS('b.created_at')} AS created_at,
     ...
     ${mine} ORDER BY b.is_open DESC, b.id DESC`, params);
  json(res, 200, {
    boards: rows.map((b) => ({
