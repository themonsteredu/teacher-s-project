...
const { createSession, destroySession, getSessionUser, cleanupSessions, parseCookies, roleLevel } = require('./auth');
...
route('PUT', /^\/api\/boards\/(\d+)\/items$/, 'teacher', async (req, res, ctx) => {
  ...
  if (!board) return notFound(res);
  if (!canManageBoard(ctx.user, board)) return forbidden(res, '이 보드를 관리할 권한이 없습니다.');
  const linkIds = Array.isArray(ctx.body?.link_ids) ? ctx.body.link_ids.map(Number).filter(Boolean) : [];
  const fileIds = Array.isArray(ctx.body?.file_ids) ? ctx.body.file_ids.map(Number).filter(Boolean) : [];
  const validLinks = new Set((await q('SELECT id FROM program_links WHERE program_id = $1', [board.program_id])).map((r) => r.id));
  const validFiles = new Set((await q('SELECT id FROM program_files WHERE program_id = $1', [board.program_id])).map((r) => r.id));
