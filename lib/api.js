...
route('GET', /^\/api\/files\/(\d+)\/open$/, 'teacher', async (req, res, ctx) => {
  ...
  try {
    ...
    if (isHtml && Number(f.size || 0) <= 3_500_000) { // Vercel 응답 한도(4.5MB) 내에서 직접 서빙
      ...
      let html = Buffer.from(await r.arrayBuffer()).toString('utf-8');
      // 프로그램 영상 주입: 교안 HTML이 window.__LESSON_VIDEO__ 를 읽어 흐름 중간에서 재생한다.
      ...
      const vid = await one(
        ...
        [f.program_id]);
      // 어떤 유튜브 링크(watch·youtu.be·shorts·live·embed·모바일)든 임베드 URL로 정규화해
      // 주입한다. 교안이 이 값을 iframe으로 그대로 띄우므로 형식과 무관하게 재생된다.
...
route('GET', /^\/api\/join-board\/([A-Za-z0-9]{4,10})\/file\/(\d+)\/open$/, null, async (req, res, ctx) => {
  ...
  try {
    ...
    if (isHtml && Number(f.size || 0) <= 3_500_000) {
      ...
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'private, no-store', 'Referrer-Policy': 'no-referrer' });
      return res.end(buf);
    }
    res.writeHead(302, { Location: url, 'Cache-Control': 'private, no-store' });
    res.end();
