...
const { handleCareerLogApi } = require('./career-log');
...
async function handleApi(req, res, pathname, body) {
  // Career Log는 중앙 API로 전달하므로 Hub 자체 DB 초기화와 독립적으로 처리한다.
  if (await handleCareerLogApi(req, res, pathname, body)) return;
