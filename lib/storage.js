...
// 한 화면에 사진이 수십 장 나온다. 한 장씩 서명하면 그만큼 왕복이 늘어나므로 묶어서 받는다.
// 묶음 발급이 안 되는 환경에서는 한 장씩으로 물러난다 — 느릴 뿐 화면은 그대로 나온다.
async function createSignedDownloads(paths, { ttl = DOWNLOAD_TTL } = {}) {
  ...
  try {
    const r = await fetch(`${SUPABASE_URL}/storage/v1/object/sign/${BUCKET}`, {
      ...
      signal: AbortSignal.timeout(15000),
      headers: authHeaders({ 'Content-Type': 'application/json' }),
      body: JSON.stringify({ expiresIn: ttl, paths: unique }),
    });
    const list = r.ok ? await r.json() : null;
    // 응답은 보낸 차례대로 온다. 길이가 어긋나면 짝을 믿을 수 없으므로 한 장씩으로 간다.
    if (Array.isArray(list) && list.length === unique.length) {
      list.forEach((item, i) => {
        const url = item?.signedURL || item?.signedUrl;
        if (url) signed.set(unique[i], full(url));
      });
      return signed;
    }
  } catch { /* 아래에서 한 장씩 발급한다 */ }
  for (const path of unique) {
    try { signed.set(path, await createSignedDownload(path, { ttl })); } catch { /* 이 장만 건너뛴다 */ }
  }
  return signed;
}
...
module.exports = { storageEnabled, ensureBucket, createSignedUpload, createSignedDownload, createSignedDownloads, inspectObject, removeObject, archiveSubmission, BUCKET };
